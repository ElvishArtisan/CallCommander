...
void MLHPIPlayStream::tickClock()
{
  ...
  LogHpi(HPI_OutStreamGetInfoEx(NULL,hpi_stream,
				&state,&buffer_size,&data_to_play,
  ...
  if(!stopping) {
	...
	return;
      ...
      left_to_write-=n;
#if HPI_VER > 0x00030500
      LogHpi(HPI_OutStreamWriteBuf(NULL,hpi_stream,pdata,n,&format));
#else
      LogHpi(HPI_DataCreate(&hpi_data,&format,pdata,n));
      LogHpi(HPI_OutStreamWrite(NULL,hpi_stream,&hpi_data));
#endif
      LogHpi(HPI_OutStreamGetInfoEx(NULL,hpi_stream,
				    &state,&buffer_size,&data_to_play,
...
hpi_err_t MLHPIPlayStream::LogHpi(hpi_err_t err)
{
  char err_txt[200];
