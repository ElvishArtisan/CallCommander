...
//
//    $Id: mlwavefile.h,v 1.5 2012/06/26 21:30:02 cvs Exp $
//
...
#define ML_MAX_YEAR 8000

/**
 * @short A class for handling Microsoft WAV files.
 * @author Fred Gleason <fredg@wava.com>
 *
 * MlwaveFile provides an abstraction of a Microsoft RIFF-based WAV file.
...
class MLWaveFile
{
 public:
  ...
  /**
   * Create an MLWaveFile object.
   * @param file_name The name of the WAV file to load into the object.
   **/
  MLWaveFile(QString file_name="");

   /**
    * Destroy an MLWaveFile object.
    **/
  ~MLWaveFile();
   ...
   **/
   bool createWave(MLWaveData *data=NULL);
   ...
   **/
   bool openWave(MLWaveData *data=NULL);
   ...
   **/
   MLWaveFile::Encoding encoding() const;

  /**
   * Set the encoding type.
   * @param code The encoding to use.
   **/
   void setEncoding(MLWaveFile::Encoding code);
   ...
   * Values currently understood by MLWaveFile are:
  ...
  private:
   MLWaveFile::Type GetType(int fd);
   bool IsWav(int);
   bool IsMpeg(int);
   bool IsOgg(int);
   bool IsAtx(int);
   ...
   unsigned energy_ptr;
   int wave_id;
   MLWaveFile::Type wave_type;

   unsigned char *cook_buffer;
   int cook_buffer_size;
   MLWaveFile::Encoding cook_encoding;
